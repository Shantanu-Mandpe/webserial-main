...
const info = () => {
  ...
  return ` - ${connection.bleName || 'Unknown Device'} (${connection.id})`
}
</script>
...
<template>
  <div id="options" :class="{start:!connection.messages.length, open:connection.open}">
    <h2>
      <strong v-if="!connection.id || !connection.bleConnected">DISCONNECTED</strong>
      <strong v-else-if="connection.id || connection.bleConnected">CONNECTED</strong>
    ...
    <fieldset>
      ...
      <!-- <button v-if="!connection.id || !connection.bleConnected" @click="connection.selectPort">Select Bluetooth Device...</button>
      <button v-else @click="connection.connect">Connect</button> -->
      <button @click="connection.selectDevice">Connect</button>
    </fieldset>
  </div>
</template>
